...
Bei der Unteruschung der Robustheit gegenüber Rauschen fällt auf, dass sowohl die Standardabweichung des MVs als auch die der Orientierung für einen $\phi\tindex{s,2}$ sehr groß wird. In dieser Stellung ist der MV in der $xy$-Ebene. In dieser Stellung liegt der MV in der $xy$-Ebene. Das Dipolmoment durchläuft zweimal pro Periode die Nulldurchgangsebene und diese Stellen werden detektiert (vgl. Abbildung\,\ref{fig:ZeroCrossingVis}). Wenn wie hier der MV in der $xy$-Ebene liegt, ist $\vec{e}\tindex{z}$ teil der Nulldurchgangsebene. Die Art der Ansteuerung sorgt dafür, dass das Dipolmoment in jeder Periode für $\omega_{\theta}\Delta t n = 0$ zu $\vec{e}\tindex{z}$ wird und so die Nulldurchgangsebene nahezu immer an derselben Stelle durchquert werden. Daher sind alle $\vec{e}\tindex{zc}=\pm\vec{e}\tindex{z}$ und das Kreuzprodukt daraus ergibt 0 oder durch das ergänzte Rauschen nahe 0. Das Normieren des MV ist dann entweder nicht definiert (Division durch 0) oder die Richtung des MV ergibt sich durch das Rauschen. Abbhilfe kann hier schaffen, die Rahmendauer zu erhöhen, sodass ein Rahmen eine volle Periode von $\omega_\phi$ beinhaltet. In diesem Fall sind zumindest 2 $\vec{e}\tindex{zc}$ unterschiedlich zu 
Diese Problematik ist in Abbildung XX dargestellt.
