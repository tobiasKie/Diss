...
\paragraph{Robustheit gegenüber Rauschen}
Bei der Unteruschung der Robustheit gegenüber Rauschen fällt auf, dass sowohl die Standardabweichung des MVs als auch die der Orientierung für einen $\phi\tindex{s,2}$ sehr groß wird. In dieser Stellung ist der MV in der $xy$-Ebene. In dieser Stellung liegt der MV in der $xy$-Ebene. Das Dipolmoment durchläuft zweimal pro Periode die Nulldurchgangsebene und diese Stellen werden detektiert und können dargestellt wie in Abbildung\,\ref{fig:ZeroCrossingVis}. Es gibt immer eine Nullebene, in der das detektierte Magnetfeld 0 ist. Problem an dieser Stelle ist jedoch, wenn der MV wie hier in der $xy$-Ebene liegt und somit $\vec{e}\tindex{z}$ ein Teil der Nullebene ist, dass diese Nullebene fast immer an der selben Stelle durchquert wird, bei $\pm\vec{e}\tindex{z}$. Die Art der Ansteuerung (Gleichung\,\ref{eq:rotDipole}) sorgt dafür, dass das Dipolmoment in jeder Periode für $\omega_{\theta}\Delta t n = 0$ zu $\vec{e}\tindex{z}$ und für $\omega_{\theta}\Delta t n = \pi$ zu $-\vec{e}\tindex{z}$. Daher sind alle $\vec{e}\tindex{zc}=\pm\vec{e}\tindex{z}$ und das Kreuzprodukt daraus ergibt 0 oder durch das ergänzte Rauschen nahe 0. Das Normieren des MV ist dann entweder nicht definiert (Division durch 0) oder die Richtung des MV ergibt sich durch das Rauschen. Abbhilfe kann hier schaffen, die Rahmendauer zu erhöhen, sodass ein Rahmen eine volle Periode von $\omega_\phi$ beinhaltet. In diesem Fall sind 2 $\vec{e}\tindex{zc}$ unterschiedlich zu $\vec{e}\tindex{z}$. Diese Problematik ist in Abbildung\,\ref{fig:MerkmalUndefined} dargestellt. Eine bessere Lösung wäre es, die Ansteuerung während der Laufzeit anzupassen. Dies könnte so umgesetzt werden, dass eine Rotation auf die Ansteuerung angewendet wird, sodass die resultierende $z$-Achse möglichst parallel zu allen MV steht. Dies wurde im Rahmen dieser Arbeit nicht mehr implementiert.
...
\end{figure}
Aus jeweils zwei Nulldurchgangsvektoren wird dann ein MV gebildet und alle werden gemittelt. Diese MV können dann nicht mehr identisch sein und nach der Mittelung kommen diese Artefakte in den MV und somit auch in die Schätzung.
...
\end{figure}
In einem Bereich bis 0.1\,cm verhält sich der Fehler linear, ein Positionsfehler von 0.05\,m führt zu einem Orientierungsfehler von ungefähr $1^\circ$.
