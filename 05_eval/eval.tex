...
\begin{figure}[h]
  ...
  \subfloat[Längerer Rahmen]{
      \scalefont{2}
      \inputTikZ{0.4}{05_eval/images/EZ_ZC_full_period.tikz}
      \label{fig:LongFrame}
  }
  \caption{(a) zeigt die $\vec{e}\tindex{zc}$s für eine Rahmenlänge 512, darin ist gut zu sehen, dass alles $\vec{e}\tindex{zc} = \pm\vec{e}\tindex{z}$sind. (b) zeigt dasselbe jedoch für eine Rahmenlänge von 4096. Dabei gibt es auch $\vec{e}\tindex{zc}$s die nicht in $z$-Richtung polarisiert sind. } 
...
\end{figure}
Aus jeweils zwei Nulldurchgangsvektoren wird dann ein MV gebildet und alle werden gemittelt. Diese MV sind nicht identisch und nach der Mittlung kommen diese Artefakte in den MV und somit auch in die Orientierungsschätzung.
...
\begin{table}[h]
  ...
  Das Kamerasystem liefert mit einer Abtastrate von 100\,Hz Positionen. Um beide System gut miteinander vergleichen zu können, wurde die Rahmenlänge der Signalverarbeitung so angepasst, dass auch die magnetische Schätzung Werte mit 100\,Hz liefert. In Abbildung\,\ref{fig:RawResults} sind die Ergebnisse der dynamischen Aufnahme ohne Nachverarbeitung dargestellt. 
  \begin{figure}[h]
    \centering
  ...
  \end{figure}
  Die rohen Schätzungen werden wie beschrieben mit einem Butterworth-Tiefpassfilter gefiltert. Die Ergebnisse nach der Filterung mit Fehlerbsetimmung für jede Schätzung sind in den Abbildungen\,\ref{fig:ResultsFirstElement} und\,\ref{fig:ResultsSecondElement} dargestellt. 
  \begin{figure}[h]
    \centering
  ...
  \end{figure}
\subsubsection{Diskussion}
Das magnetische System wurde mit einem kommerziellen Kamerasystem von \textit{Qualisys} verglichen. Die statischen Messungen zeigen den geringsten Fehler. Der Fehler liegt im Bereich von wenigen $^\circ$. Der restliche Fehler hat verschiedene Gründe, in der Modellbildung wurde sowohl das Sensormodell und das Spulenmodell vereinfacht. 

Die dynamischen Messungen ohne Nachverarbeitung zeigen, dass die magnetischen Schätzungen stärker verrauscht sind als die optischen. Mit einer kürzeren Rahmenlänge von 256 (5.3\,ms) wurden die Daten prozessiert und damit eine Frequenzanalyse durchgeführt. Diese ist in Abbildung\,\ref{fig:FFTAngles} dargestellt. 
...
\begin{figure}[h]
  ...
  \subfloat[Abweichung der mittleren Position vom Ground Truth]{
    \scalefont{2}
...
\begin{figure}[h]
  ...
  \end{overpic}
  \caption{Aufbau für die Evaluierung der Lokalisierung: Es ist ein würfelförmiger Aufbau, mit grauen PVC-Streben. An diesen sind die Sensoren(Rot, Grün und Blau) angebracht. An diesen Streben sind in jeder Richtung zwei optische Marker angebracht. }
