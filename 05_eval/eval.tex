...
Alle Messungen und Simulationen werden mit einem PC durchgeführt, nur auf der CPU.
...
\begin{figure}[h]
  ...
  \subfloat[Verhältnis der Standardabweichung]{
      \scalefont{2.0}
      \inputTikZ{0.45}{05_eval/images/FaktorVar.tikz}
      \label{fig:VarOrt} 
  }
  \caption{Die linke Abbildung zeigt den Fehler der Orientierung, nach einer Mittelung von 100 Schätzungen. Die rechte Seite zeigt das Verhältnis zwischen der Standardabweichung des MV und der Standardabweichung der Orientierung.}
  \label{fig:Variance}
\end{figure}
Um diese Ortsabhängigkeit zu zeigen wurden die beiden Varianzen zueinander ins Verhältnis gesetzt. Das Ergebnis ist in Abbildung\,\ref{fig:VarOrt} dargestellt. 
...
\begin{figure}[h]
  \centering
  \subfloat[3-Koordinatenkomponenten]{
      \scalefont{2}
...
Bei der Unteruschung der Robustheit gegenüber Rauschen fällt auf, dass sowohl die Standardabweichung des MVs als auch die der Orientierung für einen $\phi\tindex{s,2}$ sehr groß wird. In dieser Stellung ist der MV in der $xy$-Ebene. Das Dipolmoment durchläuft zweimal pro Periode die Nulldurchgangsebene und diese Stellen werden detektiert und können dargestellt wie in Abbildung\,\ref{fig:ZeroCrossingVis}. Es gibt immer eine Nullebene, in der das detektierte Magnetfeld 0 ist. Problem an dieser Stelle ist jedoch, wenn der MV wie hier in der $xy$-Ebene liegt und somit $\vec{e}\tindex{z}$ ein Teil der Nullebene ist, dass diese Nullebene fast immer an der selben Stelle durchquert wird, bei $\pm\vec{e}\tindex{z}$. Die Art der Ansteuerung (Gleichung\,\ref{eq:rotDipole}) sorgt dafür, dass das Dipolmoment in jeder Periode für $\omega_{\theta}\Delta t n = 0$ zu $\vec{e}\tindex{z}$ und für $\omega_{\theta}\Delta t n = \pi$ zu $-\vec{e}\tindex{z}$. Daher sind alle $\vec{e}\tindex{zc}=\pm\vec{e}\tindex{z}$ und das Kreuzprodukt daraus ergibt 0 oder durch das ergänzte Rauschen nahe 0. Dieses Verhalten ist in Abbildung\,\ref{fig:ShortFrame} dargestellt. Das Normieren des MV ist dann entweder nicht definiert (Division durch 0) oder die Richtung des MV ergibt sich durch das Rauschen. Abbhilfe kann hier schaffen, die Rahmendauer zu erhöhen, sodass ein Rahmen eine volle Periode von $\omega_\phi$ beinhaltet. In diesem Fall sind zwei $\vec{e}\tindex{zc}$ unterschiedlich zu $\vec{e}\tindex{z}$, wie in Abbildung\,\ref{fig:LongFrame}. Selbst wenn die längere Rahmenlänge verwendet wird, ist der Winkel zwischen den $\vec{e}\tindex{zc}$ relativ gering. Dies führt dazu, dass Rauschen einen größeren Einfluss auf das Kreuzprodukt der $\vec{e}\tindex{zc}$ hat. Abbhilfe könnte hier eine Anpassung der Ansteuersignale auf Senderseite schaffen. Die Ansteuerung ist in Gleichung\,\ref{eq:rotDipole} eingeführt. Mit einer Rotation der Ansteuerung könnte virtuell die $z$-Achse so gedreht werden, dass die $\vec{e}\tindex{MV}$ in Richtung der resultierenden $z$-Achse steht. Ein solches "Zielen" könnte die Merkmalsextraktion in diesem Bereich verbessern. Dies wurde im Rahmen dieser Arbeit nicht untersucht.
\begin{figure}[h]
  \centering
  \subfloat[Kurze Rahmenlänge]{
      \scalefont{2}
      \inputTikZ{0.4}{05_eval/images/EZ_ZC.tikz}
      \label{fig:ShortFrame} 
  }
  \subfloat[Längere Rahmenlänge]{
      \scalefont{2}
      \inputTikZ{0.4}{05_eval/images/EZ_ZC_full_period.tikz}
      \label{fig:LongFrame}
  }
  \caption{(a) zeigt die $\vec{e}\tindex{zc}$s für eine Rahmenlänge 512. (b) zeigt dasselbe für eine Rahmenlänge von 4096.} 
  \label{fig:MerkmalUndefined}
\end{figure}

In Abbildung\,\ref{fig:ErrorProj} ist der Fehler über 100 gemittelte Schätzungen dargestellt. Es ist gut zu erkennen, dass der Fehler in den meisten Stellungen gegen 0 konvergiert. Für $\phi\tindex{s,2}\approx 110$ bleibt ein verbleibender Fehler. Dieses Verhalten ist naheliegend, da das Eingangsmerkmal (MV) fehlerhaft bestimmt wurde.

Die Ergebnisse aus Abbildung\,\ref{fig:VarOrt} zeigen eine Abhängigkeit der Robustheit von der Stellung der Kette. Je näher $\phi\tindex{s,2}$ bei 0 liegt, desto größer ist die Standardabweichung/ geringer die Robustheit. Dies kann erklärt werden mit Abbildung\,\ref{fig:0_05_Unique} erklären. Man erkennt hier gut, dass für ein $\phi\tindex{max,2} = 0$ die Steigung der Kurven am steilsten im gesamten Verlauf ist. Je größer $\phi\tindex{max,2}$ wird sinkt die Steigung der Kurve. Dies lässt sich wie folgt zusammenfassen, die Störung hat Einfluss auf $\phi\tindex{max,2}$, aber je nach Stellung des Gelenks ist der Einfluss von $\phi\tindex{max,2}$ auf $\phi\tindex{s,2}$ unterschiedlich groß.
...
\begin{figure}[h]
  \centering
  \scalefont{2}
  \inputTikZ{0.4}{05_eval/images/ZeroCrossingWithMotion.tikz}
  \caption{Nulldurchgangsvektoren die aus einem der simulierten Rahmen extrahiert wurden. Für einen unbewegten Sensor ohne Rauschen liegen die Vektoren immer in einer Ebene, hier ist gut zu erkennen, dass dies für einen bewegten Sensor nicht gilt.}
...
Die statischen Messungen werden nun verwendet um die händische Kalibrierung zu verbessern. Begonnen wurde mit dem ersten Element, dafür wurden 13 Aufnahmen in unterschiedlichen Ausrichtungen verwendet. Die optimale Kalibrierung sei definiert, dass für diese die optische $\hat{\vec{e}}\tindex{opt,mag}^{\,(j)}$ und die magnetische $\hat{\vec{e}}\tindex{mag}^{\,(j)}$ Schätzung am besten miteinander übereinstimmen. Als Differenz zwischen den jeweiligen Schätzungen wird der Winkel zwischen diesen verwendet. Als Kostenfunktion ergibt sich daraus dann die folgende Gleichung, wobei $N$ die Anzahl der Messungen und $j$ die Nummer des Elements ist

\begin{equation}
    e_{j} = \frac{1}{N} \sum_{j=1}^{N} \arccos(\hat{\vec{e}}\tindex{opt,mag}^{\,(j)}\cdot\hat{\vec{e}}\tindex{mag}^{\,(j)}).
        \label{eq:CostFkt}
\end{equation}
In einem Bereich von $\pm 2\,cm$ zu den gemessenen Werten wurden in einem Raster von 0.1\,mm untersucht und die Kombination mit dem geringsten Fehler wurde übernommen. Mit den Werten des ersten Elements wurde dann das zweite Element auf die selbe Art kalibriert, hier mit $N=8$ Aufnahmen. Die Kalibriertenwerte unterscheiden sich leicht zu den händisch gemessenen Werten $\vec{r}_j = (-0.0137,0.0029,0.296)\,\text{m}$, $d\tindex{ke,1}=0.008\,\text{m}$, $l\tindex{js,1}=0.063\,\text{m}$, $d\tindex{ke,2}=0.008\,\text{m}$, $l\tindex{js,2}=0.063\,\text{m}$.
...
\begin{table}[h]
  ...
  Das Kamerasystem liefert mit einer Abtastrate von 100\,Hz Positionen. Um beide System gut miteinander vergleichen zu können, wurde die Rahmenlänge der Signalverarbeitung so angepasst, dass die magnetische Schätzung Werte mit 100\,Hz liefert. In Abbildung\,\ref{fig:RawResults} sind die Ergebnisse der dynamischen Aufnahme ohne Nachverarbeitung dargestellt. 
  \begin{figure}[h]
    \centering
  ...
  \end{figure}
\subsubsection{Diskussion}
Das magnetische System wurde mit einem kommerziellen Kamerasystem von \textit{Qualisys} verglichen. Die statischen Messungen zeigen den geringsten Fehler, wohingegen die dynamischen Messungen einen leicht größeren Fehler haben. Die Messungen haben auch gezeigt, dass das zweite Elemente einen größeren Fehler hat als das erste. Dies ist logisch, da der Fehler des ersten Elements in die Schätzung des zweiten mit einfließt. Außerdem ist das zweite Element tendenziell weiter von der Quelle entfernt und die Signalstärke wird schwächer. Der mittlere Fehler liegt in allen Szenarien im einstelligen $^\circ$-Bereich. Der verbleibende Fehler kann unterschiedliche Gründe haben, in der Modellbildung wurden zum Beispiel die Sensoren als in einem Punkt lokalisiert angenommen, die Dipolnäherung ist nicht beliebig genau oder die Sensoren haben durch den Ferritkern selsbt eine leicht verezerrende Wirkung auf das Magnetfeld. Die quantitativen Messungen haben trotzdem gezeigt, dass mit dem vorgestellten Verfahren die Bewegung einer kinematischen Kette bestimmt werden kann.
...
\begin{figure}[h]
  ...
  \subfloat[FFT der Winkelschätzung]{
    \inputTikZ{0.425}{05_eval/images/FFTAngles.tikz}
    \label{fig:FFTAngles1} 
  }
  \subfloat[FFT eines Kanals von $\bvec{b}$]{
    \inputTikZ{0.425}{05_eval/images/FFTMag.tikz}
    \label{fig:FFTMag}
  }
...
\begin{figure}[h]
  ...
  \subfloat[Abweichung der mittleren $x$-Komponente vom Ground Truth]{
    \scalefont{2}
    \inputTikZ{0.425}{05_eval/images/absoluterFehler_Pos.tikz}
    \label{fig:errorPosSim}
  } 
  \caption{Ergebnisse der Simualtion zur Überprüfung der Rebustheit der Positionsschätzung gegen weißes Rauschen}
...
Die Position der 3D-Spule geht in die Orientierungsschätzung ein. Mit einer Simulation wird überprüft, wie sich eine falsche Schätzung der Position auf die Orientierung auswirkt. Die Spule wird dafür in der Mitte des Aufbaus positioniert. Um eine falsche Position zu schätzen wird ein zufälliger räumlicher Vektor mit auf die korrekte Position addiert. Die Länge dieses Vektors wird variiert, zwischen 0\,m und 0.1\,m in 0.01\,m Schritten, und für jede Länge jeweils 100 Rahmen simuliert. Das Ergebnis ist die Standardabweichung des Orientierungsfehler in Abhängigkeit vom Positionsfehler, dargestellt in Abbildung\,\ref{fig:stdAbwOrPosVar}. 

\begin{figure}[h]
  \centering
  \scalefont{2}
  \inputTikZ{0.425}{05_eval/images/Abhängigkeit_or_Pos.tikz}
  \caption{Standardabweichung des Orientierungsfehler in Abhängigkeit vom Positionsfehler}
...
Eine einfache Bewegung wurde simuliert, dabei wurde die 3D-Spule mit einer konstanten Geschwindigkeit von $v = 0.1\,\frac{\text{m}}{\text{s}}$ der Startpunkt liegt bei $\vec{r}\tindex{c,start} = \begin{pmatrix}
  ...
  0\,\text{m} \\
  0\,\text{m}
\end{pmatrix}$. Die Bewegung wird für insgesamt $0.6\,\text{m}$ simuliert. 
\begin{figure}[h]
  \centering
  \subfloat[Vergleich $x$-Komponente gegen Ground Truth]{
    \scalefont{2}
    \inputTikZ{0.45}{05_eval/images/MovementComponent.tikz}
    \label{fig:MovementComponent}
  }
  \subfloat[Fehler im Vergleich zum Ground Truth]{
    \scalefont{2}
...
In Abbildung\,\ref{fig:stdAbwOrPosVar} ist dargestellt, wie sich im Mittel die Orientierung in Abhängigkeit von einem Positionsfehler verhält. Die extrahierten MV bleiben korrekt, nur die berechneten $\vec{e}^{\text{Glob}}\tindex{max}$ sind fehlerhaft. Die Simulation hat ergeben, dass wenn der Positionsfehler im Bereich bis zu 10\,cm liegt, der mittlere Winkelfehler sich nahezu linear verhält. Der Winkelfehler steigt dabei um etwa $1^\circ$ pro cm Positionsfehler.  
...
\begin{table}[h]
  ...
  \hline 
   $\hat{x}_{opt}$ & $\hat{x}_{mag}$ & $\hat{y}_{opt}$ & $\hat{y}_{mag}$ & $\hat{z}_{opt}$ & $\hat{z}_{mag}$ & Fehler \\ 
   \hline
...
\begin{table}[h]
  ...
  \begin{tabular}{|c| c| c| c| c| c| c|}
  \hline 
   $\hat{\phi}_{opt}$ & $\hat{\phi}_{mag}$ & $\hat{\theta}_{opt}$ & $\hat{\theta}_{mag}$ & $\hat{\psi}_{opt}$ & $\hat{\psi}_{mag}$ & Fehler \\ 
   \hline
   $84.4^\circ$ & $99.6^\circ$ & $87.5^\circ$  & $85.8^\circ$ & $82.6^\circ$ & $106.2^\circ$ & $8.6^\circ$ \\ 
   \hline
   $89.2^\circ$ & $89.9^\circ$ & $5.7^\circ$   & $3.6^\circ$ & $-172.2^\circ$& $-175.9^\circ$ & $4.4^\circ$ \\
   \hline
   $161.1^\circ$ & $176.0^\circ$ & $88.6^\circ$  & $87.3^\circ$ & $148.3^\circ$ & $175.4^\circ$ & $12.0^\circ$\\
   \hline
   $160.2^\circ$ & $-60.7^\circ$ & $88.1^\circ$ & $58.1^\circ$ & $-128.2^\circ$ & $13.2^\circ$ & $33.4^\circ$ \\
   \hline
   $120.5^\circ$ & $12.7^\circ$ & $87.8^\circ$ & $53.1^\circ$ & $76.3^\circ$ & $-49.5^\circ$ & $41.9^\circ$\\ 
   \hline
   $131.9^\circ$ & $-105.6^\circ$ & $87.8^\circ$ & $84.2^\circ$ & $71.1^\circ$ & $-167.8^\circ$& $7.2^\circ$  \\ 
   \hline
   $109.5^\circ$ & $141.8^\circ$ & $88.5^\circ$  & $84.2^\circ$ & $87.2^\circ$ & $131.1^\circ$ & $12.5^\circ$ \\
   \hline
   $136.2^\circ$ & $151.9^\circ$ & $88.1^\circ$ & $77.1^\circ$ & $67.1^\circ$ & $89.6^\circ$ & $13.0^\circ$ \\
   \hline
   $45.4^\circ$ & $-147.3^\circ$ & $87.2^\circ$ & $89.1^\circ$ & $43.7^\circ$ & $-139.6^\circ$ & $9.9^\circ$ \\
   \hline
   $170.0^\circ$ & $156.2^\circ$ & $88.4^\circ$  & $82.0^\circ$ & $150.9^\circ$ & $156.2^\circ$ & $20.1^\circ$ \\ 
   \hline
   $-154.4^\circ$ & $-134.9^\circ$ & $87.9^\circ$  & $79.7^\circ$ & $-133.9^\circ $ & $-119.8^\circ$ & $9.8^\circ$ \\ 
   \hline
   $141.2^\circ$ & $-144.2^\circ$ & $88.7^\circ$ & $81.3^\circ$ & $168.5^\circ$ & $-99.0^\circ$ & $19.8^\circ$ \\
   \hline
  \end{tabular}
...
\begin{figure}[h]
  \centering
  \scalefont{2}
  \inputTikZ{0.4}{05_eval/images/DynEvalCompsPos.tikz}
...
\begin{figure}[h]
  ...
  \subfloat[$\theta$]{
    \scalefont{2}
    \inputTikZ{0.4}{05_eval/images/DynEvalAngleTheta.tikz}
    \label{fig:DynTheta}
  }
  \\
  \subfloat[$\phi$]{
    \scalefont{2}
    \inputTikZ{0.4}{05_eval/images/DynEvalAnglePhi.tikz}
    \label{fig:DynPhi}
  } 
  \\
  \subfloat[$\psi$]{
    \scalefont{2}
    \inputTikZ{0.4}{05_eval/images/DynEvalAnglePsi.tikz}
    \label{fig:DynPsi}
  } 
